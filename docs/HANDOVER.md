...
| CS `/cs` | 노션 CS DB. 서비스 칩, 기본 미해결, 제목→상세에서 고객명·아이디·상태·답변·비고·피드백 | 부하와 같은 `canDashboard`. **본부 격리 없음** |
...
부하 보기 작업 목록 열: 상태 · 서비스 · **업무속성** · **중요도** · 작업 · 담당 · 일정 · 진척 · 잔여. 작업 칸은 **상위 제목 경로 + 리프 제목 + 내용/이슈**. 목록은 **미지정 / 담당 지정** 구간으로 나누고, 구간마다 페이지네이션(10/20/50/100건)을 둔다. 상태·서비스 칸은 한 줄로 유지. 잔여·히트맵은 일정 일수(추가 일정 반영)와 투입률을 곱함. 소요일은 산정에 쓰지 않음. 히트맵은 **주간 / 일간**(주 단위 월–금).
...
| 지연사유 / 지연 사유 | rich_text / select | `delayReason`. 업무 상세에 표시 |
...
- **잔여 공수:** `일정 일수(시작~연장 종료, 양끝 포함) × (1 − 진척도) × 투입률`. 진척·투입률은 0–1이면 그대로, 100이면 1. 투입률이 비어 있으면 **100%**. **소요일은 산정에 쓰지 않음.**
- 추가일정이 양수면 종료일(`end` 없으면 `start`)에 그 일수만큼 더한다. 기한초과 판정·잔여·히트맵·캘린더 막대는 이 연장 종료일을 쓴다.
- 원래 종료일이 지났고 연장 종료일은 아직이면 상태는 **기한연장**. 연장 종료일까지 지났으면 **기한초과**. 슬랙 기한초과 DM은 `기한초과`만.
- 일정 없음: 미정 업무로 보고 부하·잔여 공수에서 제외 (0). 목록/캘린더에는 표시.
- 담당자 여러 명: 잔여를 인원수로 균등 분배. 담당 없으면 히트맵에서 **서비스별 미지정 행** (`(미지정) · 서비스`). 서비스 없으면 `서비스없음`. 인원별 부하 현황·KPI 담당자 수에는 넣지 않음.
- 주간 히트맵: 오늘~연장 종료일의 **평일**에 균등 배분. 주말 제외. 기한 지난 잔여는 **이번 주(week 0)** 에 합산. 일정 없는 잔여는 `unscheduledDays`.
...
11. CS 상세 저장은 `PATCH /api/cs/[id]`. 답변·비고·피드백은 노션에 해당 열이 있어야 한다. 앱은 열을 만들지 않는다.

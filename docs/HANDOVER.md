...
부하 보기 작업 목록 열: 상태 · 서비스 · **속성** · **중요도** · 작업 · 담당 · 일정 · 진척 · 잔여. 작업 칸은 **상위 제목 경로 + 리프 제목 + 내용/이슈**. 목록은 **미지정 / 담당 지정** 구간으로 나누고, 구간마다 페이지네이션(10/20/50/100건)을 둔다. 상태·서비스 칸은 한 줄로 유지. 잔여·히트맵은 일정 일수(추가 일정 반영)와 투입률을 곱함. 소요일은 산정에 쓰지 않음. 히트맵은 **주간 / 일간**(주 단위 월–금).
